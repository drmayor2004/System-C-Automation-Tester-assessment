...
//Count total number of Testcases in Retrieve All Product
...
GlobalVariable.testCaseName = "Retrieve - All Products";  //Set testName
...
GlobalVariable.testName = 'Retrieve - All Products';
GlobalVariable.testCase = testCases
GlobalVariable.tc = tc
GlobalVariable.endPointData = getURL;
...
RequestObject getAllProducts = findTestObject('Get All Product', [('url') : getURL]);
//Getting the actual url and jsonBody values to spreadsheet

//Getting the actual url and jsonBody values to spreadsheet
GlobalVariable.actualUrl = getAllProducts.getRestUrl();
GlobalVariable.jsonBody = "N/A"

//Sending request body to endpoint url
response = WS.sendRequest(getAllProducts);
